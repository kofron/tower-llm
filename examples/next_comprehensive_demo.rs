...
#[path = "../src/next/budgets/mod.rs"]
mod budgets;

#[path = "../src/next/resilience/mod.rs"]
mod resilience;

#[path = "../src/next/observability/mod.rs"]
mod observability;
...
async fn main() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    let policy = next::CompositePolicy::new(vec![
        next::policies::until_no_tool_calls(),
        next::policies::max_steps(3),
        budgets::budget_policy(budgets::Budget {
    ...
    let mut basic_agent = next::Agent::builder(client.clone())
        .model("gpt-4o-mini")
        .temperature(0.7)
        .tool(calculator)
        .policy(policy)
    ...
    let flaky_agent = tower::service_fn(
        move |req: async_openai::types::CreateChatCompletionRequest| {
            ...
            async move {
                if count == 0 {
                    // First call fails
                    Err::<next::StepOutcome, tower::BoxError>("Temporary failure".into())
                } else {
                    // Subsequent calls succeed
                    Ok(next::StepOutcome::Done {
    ...
    let retry_policy = resilience::RetryPolicy {
        max_retries: 2,
        backoff: resilience::Backoff::fixed(Duration::from_millis(100)),
    };

    let resilient_agent = resilience::TimeoutLayer::new(Duration::from_secs(5)).layer(
        resilience::RetryLayer::new(retry_policy, resilience::AlwaysRetry).layer(flaky_agent),
    );
    ...
    let collector = tower::service_fn(move |record: observability::MetricRecord| {
        let metrics = metrics_clone.clone();
        async move {
            match record {
                observability::MetricRecord::Counter { name, value } => {
    ...
    let observable_agent = observability::TracingLayer::new().layer(
        observability::MetricsLayer::new(collector).layer(tower::service_fn(
