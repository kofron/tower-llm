...
impl Service<observability::MetricRecord> for DemoMetricsCollector {
    type Response = ();
    ...
    fn call(&mut self, req: observability::MetricRecord) -> Self::Future {
        ...
        Box::pin(async move {
            match req {
                ...
                observability::MetricRecord::Histogram { name, value } => {
...
async fn main() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    // Add tracing layer
    let tracing_layer = observability::TracingLayer::new();
    ...
    // Add metrics layer
    let metrics_layer = observability::MetricsLayer::new(collector.clone());
    ...
    let combined = observability::TracingLayer::new()
        .layer(observability::MetricsLayer::new(combined_collector.clone()).layer(mock_step));
