...
async fn main() {
    ...
    let token_budget = budgets::Budget {
    ...
    let tool_budget = budgets::Budget {
    ...
    let time_budget = budgets::Budget {
        max_prompt_tokens: None,
        max_completion_tokens: None,
        max_tool_invocations: None,
    ...
    let combined_budget = budgets::Budget {
    ...
    let token_policy = budgets::budget_policy(token_budget);
    ...
    for step in 1..=5 {
        ...
        let state = next::LoopState { steps: step };
        let outcome = next::StepOutcome::Next {
            messages: vec![],
            aux: next::StepAux {
                prompt_tokens: 250,
    ...
    let tool_policy = budgets::budget_policy(tool_budget);
    ...
    let combined_policy = budgets::budget_policy(combined_budget);
