...
async fn main() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    let store = sessions::InMemorySessionStore::default();
    let session_id = sessions::SessionId("user_123_session".to_string());
    ...
    let memory_layer = sessions::MemoryLayer::new(
        Arc::new(store.clone()),
        Arc::new(store.clone()),
        session_id.clone(),
    );
    ...
    let stored_history = Service::<sessions::LoadSession>::call(
        &mut store_svc,
    ...
    let new_session_id = sessions::SessionId("user_456_session".to_string());
    let memory_layer2 = sessions::MemoryLayer::new(
        Arc::new(store.clone()),
        Arc::new(store.clone()),
