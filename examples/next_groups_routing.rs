...
async fn main() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    // Math agent factory
    let create_math_agent = || -> next::AgentSvc {
        tower::util::BoxService::new(tower::service_fn(
            |_req: async_openai::types::CreateChatCompletionRequest| async move {
                ...
                Ok::<_, tower::BoxError>(next::AgentRun {
                    messages: vec![
                    ...
                    steps: 1,
                    stop: next::AgentStopReason::DoneNoToolCalls,
                })
            },
        ))
    };
    ...
    let create_writing_agent = || -> next::AgentSvc {
        tower::util::BoxService::new(tower::service_fn(
            |_req: async_openai::types::CreateChatCompletionRequest| async move {
    ...
    // Code agent factory
    let create_code_agent = || -> next::AgentSvc {
        tower::util::BoxService::new(tower::service_fn(
            |_req: async_openai::types::CreateChatCompletionRequest| async move {
    ...
    let content_picker = tower::service_fn(|req: groups::PickRequest| async move {
    ...
    let router = groups::GroupBuilder::new()
    ...
    let round_robin_picker = {
        ...
        tower::service_fn(move |_req: groups::PickRequest| {
    ...
    let round_robin_router = groups::GroupBuilder::new()
        .agent("math", create_math_agent())
        .agent("writing", create_writing_agent())
        .agent("code", create_code_agent())
    ...
    let load_balanced_router = groups::GroupBuilder::new()
        .agent("math", create_math_agent())
        .agent("writing", create_writing_agent())
        .agent("code", create_code_agent())
        .picker(load_picker)
