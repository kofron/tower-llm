        ...
        Box::pin(async move {
            ...
            match &out {
                StepOutcome::Next {
                    ...
                    info!(prompt = aux.prompt_tokens, completion = aux.completion_tokens, tools = aux.tool_invocations, invoked = ?invoked_tools, "step next")
                }
                StepOutcome::Done { aux, .. } => info!(
                    prompt = aux.prompt_tokens,
                    completion = aux.completion_tokens,
                    tools = aux.tool_invocations,
                    "step done"
                ),
            }
            Ok(out)
...
pub struct MetricsLayer<C> {
    collector: C,
}
impl<C> MetricsLayer<C> {
        ...
        Box::pin(async move {
            ...
            let (prompt, completion, tools) = match &out {
                StepOutcome::Next { aux, .. } | StepOutcome::Done { aux, .. } => (
                    aux.prompt_tokens,
                    aux.completion_tokens,
                    aux.tool_invocations,
                ),
            };
            let _ = ServiceExt::ready(&mut collector)
                .await?
...
mod tests {
    ...
    async fn metrics_layer_updates_collector() {
        ...
        let collector = service_fn(move |rec: MetricRecord| {
            ...
            async move {
                if let MetricRecord::Counter { name, value } = rec {
                    sink.lock().await.push((name, value))
                }
                Ok::<(), BoxError>(())
            }
        });
        ...
        assert!(data
            .iter()
            .any(|(n, v)| *n == "completion_tokens" && *v == 7));
        assert!(data
            .iter()
            .any(|(n, v)| *n == "tool_invocations" && *v == 1));
    }
}
