...
Use the compiler, clippy, and tests to tell you if you're done.  You shouldn't consider your task finished if everything isn't green.  Obviously in the middle you might have broken tests or lint, but before you turn control back over to the user, you should leave things fully green.
...
## Consider the Mandala
Code gets messy - it's a reality.  There are times when trying to refactor or fix an existing implementation is a lot harder than extracting a sound interface and then doing a nice clean rewrite & delete the old stuff.  In general when you are refactoring/adding features/fixing bugs, you should consider this approach - PARTICULARLY if it's proving super difficult to make progress with the current codebase.

The side benefit of this is that you can often "discover" cleaner interfaces/types/etc that can make the code more expressive and well-factored.

Be judicious, but not afraid!
